...
##            print 'aaaa ',self.rb_user
##            print 'bbbb ',self.rb_pass
...
class ReviewBoardServer(object):
    ...
    def save_comment_draft(self, api_link):
        """
        Saves a draft of review comment
        """
        self.api_post("%s/save/" % api_link)

    def reply_comment_draft(self, api_link, valueDict):
        """
        Add comment to existing comments as a reply
        """
        self.api_post("%s"%api_link, valueDict)
...
##            print file(self.cookie_file).read()
...
##        print('HTTP POSTing to %s: %s' % (url, debug_fields))
...
class SVNClient(SCMClient):
    ...
    def get_repository_info(self):
        ...
        svnCmd = "svn info --username jirauser --password P@ssw0rd"
        svnCmd = svnCmd + " " + options.repository_url
        data = execute(svnCmd,
                       ignore_errors=True)
...
##            print 'new_url : ',new_url
            ...
            return self.do_diff(["svn", "diff", old_url,
                                 new_url] + files,
                                repository_info)
...
def get_repository_info(strRepoUrl):

        # Get the SVN repository path (either via a working copy or
        # a supplied URI)
        svn_info_params = ["svn", "info"]
        svnCmd = "svn info --username jirauser --password P@ssw0rd"
##        svnCmd = "svn info --username " + options.username + " --password " + options.password
        svnCmd = svnCmd + " " + strRepoUrl
        data = execute(svnCmd,
                       ignore_errors=True)
        m = re.search(r'^Repository Root: (.+)$', data, re.M)
        if not m:
...
##            server.login()
            ...
            sys.stderr.write(r'Unable to find Reviewboard server')
