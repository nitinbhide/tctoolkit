...
##        self.wdgt.bind( '<Motion>', self.move, '+' )
        ...
        for objid in objids:
            ...
            for tag in tags:
                if( tag.startswith('tooltip:')):
                    self.wdgt.tag_bind(objid, '<Enter>', lambda e, id=objid: self.spawn(e, id), '+')
    ...
    def spawn( self, event=None, objid=None):
        ...
        try:
            ...
            self.msgVar.set(msg)                                   # Try to call the message function.  Will not change the message if the message function is None or the message function fails
